...
const LOGIN_REDIRECT_ALLOWLIST = new Set([
  'index.html',
  'admin.html',
  'profile.html',
  'messages.html',
...
function sanitizeLoginRedirectTarget(rawTarget) {
  if (!rawTarget) return 'index.html';
  try {
    const url = new URL(rawTarget, window.location.href);
    if (url.origin !== window.location.origin) return 'index.html';
    const page = url.pathname.split('/').pop() || 'index.html';
    if (!LOGIN_REDIRECT_ALLOWLIST.has(page)) return 'index.html';
...
function getLoginRedirectTarget() {
  ...
  return sanitizeLoginRedirectTarget(params.get('next') || params.get('redirect'));
}
...
function loadCaptchaWidget(containerId) {
  ...
  window._pendingCaptchaContainers = window._pendingCaptchaContainers || [];
  if (!window._pendingCaptchaContainers.includes(containerId)) {
    window._pendingCaptchaContainers.push(containerId);
  }
  ...
  if (typeof hcaptcha === 'undefined') {
    ...
    script.async = true;
    script.onerror = () => {
      const pending = window._pendingCaptchaContainers || [containerId];
      captchaType = 'math';
      captchaSitekey = null;
      hcaptchaLoaded = false;
      pending.forEach(id => loadMathCaptcha(id));
      window._pendingCaptchaContainers = [];
    };
...
window.onHCaptchaLoad = function() {
  hcaptchaLoaded = true;
  const pending = window._pendingCaptchaContainers || [];
  pending.forEach(id => renderCaptchaIn(id));
  window._pendingCaptchaContainers = [];
};
...
async function doLogin() {
  ...
  try {
    ...
    // LOGIN SUCCESS
    // ============================================================
    setAuthSession(data.token, data.user, data.csrfToken);

    // Show 2FA warning for admins if not set up
    if (data.warning2FA) {
      finishLogin('Welcome back, ' + data.user.displayName + '! Note: ' + data.warning2FA);
    } else {
      finishLogin('Welcome back, ' + data.user.displayName + '!');
    }
  } catch (err) {
...
async function submitTotp() {
  ...
  try {
    ...
    pending2FA = null;
    setAuthSession(data.token, data.user, data.csrfToken);
    finishLogin('Welcome back, ' + data.user.displayName + '! (2FA verified)');
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  fetch(GATEWAY_URL + '/api/auth/captcha-config')
    ...
    .then(data => {
      ...
      if (captchaEnabled) {
        loadCaptchaByType('signupCaptchaRow');
        if (captchaType === 'math' || captchaSitekey) {
          loadCaptchaByType('loginCaptchaRow');
        }
      }
  ...
  if (hasAuthSession()) {
    fetch(GATEWAY_URL + '/api/auth/me', { headers: authHeaders(false), cache: 'no-store' })
      .then(async r => {
        if (r.ok) {
