...
// 5.4.4: lazy-load Chart.js. Injects the CDN script the first time it's
// needed (when an admin chart enters viewport), so non-admin pages never
// download ~200 KB of charting code.
function ensureChartJsLoaded() {
  if (typeof Chart !== 'undefined') return Promise.resolve();
  if (ensureChartJsLoaded._p) return ensureChartJsLoaded._p;
  ensureChartJsLoaded._p = new Promise((resolve, reject) => {
    const s = document.createElement('script');
    s.src = 'https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.js';
    s.async = true;
    s.onload = () => resolve();
    s.onerror = () => { ensureChartJsLoaded._p = null; reject(new Error('Chart.js failed to load')); };
    document.head.appendChild(s);
  });
  return ensureChartJsLoaded._p;
}
...
async function loadAnalytics() {
  try {
    ...
    try {
      ...
      setChartFallbacks('Chart renderer offline; operational tables remain active.');
      return;
    }
    setChartFallbacks('');
...
async function loadAnomalies() {
  try {
    ...
    visible.forEach(a => {
      const sevColor = a.severity >= 0.8 ? 'var(--red)' : a.severity >= 0.5 ? 'var(--amber)' : a.severity >= 0.25 ? 'var(--accent)' : 'var(--text2)';
      // 5.3.8: severity icon — filled circle in the matching tier color, sized
      // by tier so it pops in scan-mode. Quarantine-tier (>0.8) gets a ring.
      const sevIcon = a.severity >= 0.8
        ? `<span title="auto-quarantine threshold" style="display:inline-flex;align-items:center;justify-content:center;width:14px;height:14px;border-radius:50%;background:${sevColor};box-shadow:0 0 0 2px rgba(255,95,135,.25),0 0 6px ${sevColor};margin-right:6px"></span>`
        : `<span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${sevColor};margin-right:6px;vertical-align:middle"></span>`;
      const sevBar = `<div style="display:flex;align-items:center;gap:4px">${sevIcon}<div style="width:42px;height:4px;border-radius:2px;background:var(--border)"><div style="width:${Math.round(a.severity*100)}%;height:100%;border-radius:2px;background:${sevColor}"></div></div><span style="font-size:9px;color:${sevColor}">${(a.severity*100).toFixed(0)}%</span></div>`;
