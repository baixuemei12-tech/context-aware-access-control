...
(function () {
  ...
  /* ============================================================
     3. TRUST ORB — live EMA + draining RGCA budget ring
     ============================================================ */
  function initTrustOrb() {
    var orb = $('#profileTrustOrb');
    if (!orb) return;
    var valEl = $('#profileTrustValue');

    // Read whatever value the page already set (or fall back to mocked .71).
    function readTrust() {
      var raw = (valEl && valEl.textContent || '').trim();
      var n = parseFloat(raw);
      if (isFinite(n) && n > 1) n = n / 100;
      if (!isFinite(n)) n = 0.71;
      return Math.max(0, Math.min(1, n));
    }

    // Inject meta strip (DT_score / P_eff / tier) below the orb's label
    var card = orb.closest('.trust-orb-card');
    if (card && !card.querySelector('.trust-orb-meta')) {
      var meta = document.createElement('div');
      meta.className = 'trust-orb-meta';
      meta.innerHTML =
        '<div>DT<b id="fx-dt">0.71</b></div>' +
        '<div>P_eff<b id="fx-peff">0.55</b></div>' +
        '<div>tier<b id="fx-tier" style="color:var(--green)">A</b></div>' +
        '<div>budget<b id="fx-budget">100%</b></div>';
      card.appendChild(meta);
    }

    var trust = readTrust();
    orb.style.setProperty('--trust-angle', (trust * 360) + 'deg');
    orb.style.setProperty('--fx-trust-angle', (trust * 360) + 'deg');
    if (valEl && valEl.textContent.trim() === '—') valEl.textContent = trust.toFixed(2);
    ...
    setInterval(function () {
      ...
      var jitter = trust + (Math.random() * 0.018) * dir;
      dir *= -1;
      jitter = Math.max(0, Math.min(1, jitter));
      orb.style.setProperty('--fx-trust-angle', (jitter * 360) + 'deg');
      orb.style.setProperty('--trust-angle', (jitter * 360) + 'deg');
      if (valEl) valEl.textContent = jitter.toFixed(2);
      var dt = $('#fx-dt'); if (dt) dt.textContent = jitter.toFixed(2);
    }, FX_FULL ? 3500 : 5000);

    // RGCA budget — drains, resets, alarms
    orb.classList.add('fx-budget');
    var alpha = trust > 0.7 ? 0.07 : trust > 0.5 ? 0.05 : 0.03;
    var remaining = 1;
    setInterval(function () {
      if (document.hidden) return;
      remaining -= alpha;
      if (remaining < 0) remaining = 1; // re-eval cycle
      orb.style.setProperty('--fx-budget-angle', (remaining * 360) + 'deg');
      orb.classList.toggle('budget-low', remaining < 0.4 && remaining >= 0.18);
      orb.classList.toggle('budget-crit', remaining < 0.18);
      var b = $('#fx-budget'); if (b) {
        b.textContent = Math.round(remaining * 100) + '%';
        b.style.color = remaining < 0.18 ? 'var(--red)'
                      : remaining < 0.4  ? 'var(--amber)' : 'var(--neon, #38f4df)';
      }
    }, FX_FULL ? 1000 : 1800);
  }
